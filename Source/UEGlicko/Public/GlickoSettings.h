...
public:
	...
	UPROPERTY(config, EditAnywhere, BlueprintReadOnly, meta = (Keywords = "Glicko Default Rating"), Category = "Default Rating")
	double DefaultRating; // 1500.0;

	/// @brief The default/initial deviation value; φ (phi)
	UPROPERTY(config, EditAnywhere, BlueprintReadOnly, meta = (Keywords = "Glicko Default Deviation"), Category = "Default Rating")
	double DefaultDeviation; // 350.0;

	/// @brief The default/initial volatility value; σ (sigma)
	UPROPERTY(config, EditAnywhere, BlueprintReadOnly, meta = (Keywords = "Glicko Default Volatility"), Category = "Default Rating")
	double DefaultVolatility; // 0.06;

	/// @brief The Glicko-1 to Glicko-2 scale factor
	UPROPERTY(config, EditAnywhere, BlueprintReadOnly, meta = (Keywords = "Glicko Scale"))
	double Scale; // 173.7178;

	/// @brief The system constant; τ (tau)
	UPROPERTY(config, EditAnywhere, BlueprintReadOnly, meta = (Keywords = "Glicko System Constant"))
	double SystemConst; // 0.5;

	/// @brief The convergence constant; ε (epsilon)
	UPROPERTY(config, EditAnywhere, BlueprintReadOnly, meta = (Keywords = "Glicko Convergence"))
